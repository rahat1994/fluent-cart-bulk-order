...
- PR #9 — registers both filters with the rationale comment and adds `fcbo_validate_cart_item_rules()` / `fcbo_validate_checkout_minimum()` in `fluent-cart-bulk-order.php`. **Open, not merged as of this writing.** Prefer locating the code by symbol rather than by line: the repo-side line numbers in this note are accurate as of writing but drift with every edit to that file.
